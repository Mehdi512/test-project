...
CREATE TABLE `order_internal` (
  ...
  `create_timestamp` datetime DEFAULT NULL,
  `last_update_timestamp` datetime DEFAULT NULL,
  PRIMARY KEY (`order_internal_id`,`create_timestamp`),
...
CREATE TABLE `invoice` (
  ...
  `payment_agreement` varchar(50) DEFAULT '' COMMENT 'CREDIT/UPFRONT/CONSIGNMENT etc.',
  `due_date` datetime DEFAULT NULL COMMENT 'Due date for credit payments',
  `defaulter_report_status` VARCHAR(20) DEFAULT NULL COMMENT 'Status of defaulter reporting (PENDING, SUCCESS, FAILED)',
  `defaulter_report_attempt_date` TIMESTAMP DEFAULT NULL COMMENT 'Last attempt date of defaulter reporting',
  `defaulter_report_error` VARCHAR(255) DEFAULT NULL COMMENT 'Error message if defaulter reporting failed',
  `status` varchar(50) NOT NULL COMMENT 'Paid/Due/NA etc.',
  `data` longtext DEFAULT NULL,
  `create_timestamp` datetime NOT NULL,
  `last_update_timestamp` datetime DEFAULT NULL,
  PRIMARY KEY (`invoice_id`,`create_timestamp`),
  UNIQUE KEY `order_id_seller` (`order_id`,`seller`),
  KEY `idx_defaulter_report_status` (`defaulter_report_status`),
  KEY `idx_defaulter_report_attempt_date` (`defaulter_report_attempt_date`),
...
CREATE TABLE IF NOT EXISTS `order_type_category` (
  ...
  CONSTRAINT chk_order_category CHECK (order_category IN ('RETURN', 'REVERSAL', 'ORDER', 'STOCK_TRANSFER','MANUAL_ADJUSTMENT','PURCHASE_ORDER','MOVE_ORDER'))
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
