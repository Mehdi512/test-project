...
{
    private static final Logger LOGGER = LoggerFactory.getLogger(UserAttributesMapping.class);
    @Override
    User mapUserAttributes(Attributes attributes, String userDn) {
        LOGGER.info("UserAttributesMapping Attribute Size: {}", attributes.size());
        LOGGER.info("UserAttributesMapping Attribute : {}", attributes.getAll());
        User user = new User();
