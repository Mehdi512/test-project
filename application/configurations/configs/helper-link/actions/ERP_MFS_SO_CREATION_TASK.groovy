...
class ERP_MFS_SO_CREATION_TASK extends AbstractAction {
    private static final Logger log = LoggerFactory.getLogger(ERP_MFS_SO_CREATION_TASK.class)
    ...
    private static final String TASK = ERP_MFS_SO_CREATION_TASK.class.name
    @Override
    ...
    void createRequest(HelperTransaction transaction) {
        ...
        try {
            ...
            log.info("Received MFS SO REQUEST: {}", request.SALESORDERS.MFS)
            if(request.SALESORDERS.MFS != null)
            {
                setDefaultSalesOrderVariables(request.SALESORDERS.MFS)
                String salesOrderRequest = gson.toJson(request.SALESORDERS.MFS)
                transaction.getLinkActionInProgress().setRequest(salesOrderRequest)
            }
            else
            {
                log.info("\n ==================\n NO MFS SALES ORDERS IN REQUEST TO PROCESS for requestId: [{}] - inboundTxnId: [{}]\n ==================", transaction.getInboundRequest().getInboundTxnId(), transaction.getInboundRequest().getInboundTxnId())
                throw new SkippableActionException("NO MFS SALES ORDERS IN REQUEST TO PROCESS", LinkActionStatus.SKIPPED)
            }
    ...
    void makeApiCall(HelperTransaction transaction) {
        ...
        try {
            ...
            log.info("MFS SO Request to ERPLink: [{}]", salesOrderRequest)
    ...
    void callback(HelperTransaction transaction) {
        ...
        try {
            ...
            CreateSalesOrderRequest MFS =  gson.fromJson(request, CreateSalesOrderRequest.class)

            ExtUpdateOrderRequest extUpdateOrderRequest = new ExtUpdateOrderRequest()
            extUpdateOrderRequest.orderId = MFS.orderId
            extUpdateOrderRequest.tokenObj = transaction.getToken()
            extUpdateOrderRequest.resellerId = MFS.buyingPartyId
            ...
            salesOrderItem.salesOrderType = SalesOrderType.MFS
    ...
    static setDefaultSalesOrderVariables(CreateSalesOrderRequest request){
        ...
        request.transactionTypeCode = "GP Utility Bill Payment"
