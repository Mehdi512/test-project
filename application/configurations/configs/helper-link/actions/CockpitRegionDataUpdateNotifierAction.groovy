...
@CompileStatic
class CockpitRegionDataUpdateNotifierAction extends AbstractAction {
    private static final Logger log = LoggerFactory.getLogger(CockpitRegionDataUpdateNotifierAction.class)
    ...
    protected void loadActionData(HelperTransaction transaction) {
        ...
        if (!initialized.getAndSet(true)) {
            log.info("Initializing ActionData for CockpitRegionDataUpdateNotifierAction bean")
    ...
    protected void createRequest(HelperTransaction transaction) throws ActionProcessingException {
        log.info("Started creation of request for requestId: {} with inboundTxnId: {}", 
                transaction.getInboundRequest().getId(), transaction.getInboundRequest().getInboundTxnId())
        try {
            String jsonRequest = createJsonRequestString(commonUtils.getSystemToken(transaction.getInboundRequest().getToken()), transaction)
    ...
    protected void handleResponse(HelperTransaction transaction) throws ActionProcessingException, FailedAtExternalSystemException, PayloadParsingException, RetriableException {
        //Throw RetriableException if we want to retry the API call
        ...
        if(RETRIABLE_STATUS_CODES.contains(responseMap.get(STATUS_CODE).toString())) {
            ...
            log.debug("Response from Cockpit: {}", errorMessage)
    ...
    private String createJsonRequestString(SystemToken systemToken, HelperTransaction transaction) throws ActionProcessingException {
        ...
        actionHelperUtils.addParamsToJsonRequest(jsonRequestMap, "infoType", "ORG")
        ...
        actionHelperUtils.addParamsToJsonRequest(jsonRequestMap, "resourceIds", new String[]{})
    ...
    private String transformToChangeType(String operationType) throws ActionProcessingException {
        switch (operationType) {
            case "createRegion":
            case "createRegionType":
            case "addRegionMapping":
                return "CREATED"
            case "updateRegion":
            case "updateRegionType":
            case "updateRegionMapping":
                return "UPDATED"
            case "DELETE_LOCATION":
            case "DELETE_LOCATION_TYPE":
            case "DELETE_THANA_MAPPING":
                return "DELETED"
            default:
