...
@CompileStatic
class MFSPinChangeAction extends AbstractAction
{
	private static final Logger log = LoggerFactory.getLogger(MFSPinChangeAction.class);
	...
	protected void loadActionData(HelperTransaction transaction) {
		if (!initialized.getAndSet(true)) {
			log.info("Loading action data for MFSPinChangeAction, requestId: {}", transaction.getInboundRequest().getId());
			...
			log.info("Action data loaded: API_ENDPOINT={}, RETRY_DELAY_MS={}, STATUS_CODE={}, MESSAGE={}",
					API_ENDPOINT, RETRY_DELAY_MS, STATUS_CODE, MESSAGE);
		}
	...
	private Element createCommandElement(String operationType, Document doc, SystemToken systemToken, Map<String, Object> payloadMap) {
		...
		String newPin = actionHelperUtils.getMapValue(payloadMap, "newPin", true, "");
		String decodedNewPin = decodeBase64Encoded(newPin)

		String confirmPin = actionHelperUtils.getMapValue(payloadMap, "confirmPin", true, "");
		String decodedConfirmPin = decodeBase64Encoded(confirmPin)

		String msisdn = actionHelperUtils.getMapValue(payloadMap, "msisdn", true, "");

		actionHelperUtils.addElement(doc, commandElement, "TYPE", "RCPNREQ");
		...
		actionHelperUtils.addElement(doc, commandElement, "NEWPIN", decodedNewPin);
		actionHelperUtils.addElement(doc, commandElement, "CONFIRMPIN", decodedConfirmPin);
