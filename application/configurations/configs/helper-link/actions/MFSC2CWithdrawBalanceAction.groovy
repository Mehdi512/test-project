...
@CompileStatic
class MFSC2CWithdrawBalanceAction extends AbstractAction {
    private static final Logger log = LoggerFactory.getLogger(MFSC2CWithdrawBalanceAction.class);
    ...
    protected void loadActionData(HelperTransaction transaction) {
        if (!initialized.getAndSet(true)) {
            log.info("Loading action data for MFSC2CWithdrawBalanceAction, requestId: {}", transaction.getInboundRequest().getId());
    ...
    private Element createCommandElement(String operationType, Document doc, SystemToken systemToken, Map<String, Object> payloadMap) {
        ...
        actionHelperUtils.addElement(doc, commandElement, "TYPE", "RTMREQ");
        ...
        String targetMsisdn = actionHelperUtils.getMapValue(payloadMap, "msisdn", true, "");
        ...
        actionHelperUtils.addElement(doc, commandElement, "MSISDN", initiatorMsisdn);
        actionHelperUtils.addElement(doc, commandElement, "MSISDN2", targetMsisdn);
        actionHelperUtils.addElement(doc, commandElement, "AMOUNT", amount);
