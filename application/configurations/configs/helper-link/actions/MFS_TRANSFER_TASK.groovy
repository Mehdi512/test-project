...
class MFS_TRANSFER_TASK extends AbstractAction {
    private static final Logger log = LoggerFactory.getLogger(MFS_TRANSFER_TASK.class)
    private static final String EXTERNAL_CALL_RESULT_CODE = "EXTERNAL_CALL_RESULT_CODE"

    private static final String transferUrl = "http://svc-idm-mfs-link:15600/mfslink/v1/transfer-request"
    private static final String callbackUrl = "http://svc-order-management:9595/oms/v2/ext/order"
    private static final String mfsUrl = "http://{{ .Values.HOST__link_simulator }}:{{ .Values.PORT__link_simulator }}/restservice/DmsIncomingRequest"

    private static final String TASK = MFS_TRANSFER_TASK.class.name
    ...
    void createRequest(HelperTransaction transaction) {
        ...
        try {
            ...
            log.info("MFS TRANSFER REQUEST: {}", request.MFSTRANSFER)
            if(request.MFSTRANSFER != null)
            {
                String xmlRequest = createXmlRequest(request.MFSTRANSFER[0], commonUtils.getSystemToken(transaction.getInboundRequest().getToken()))
                log.debug("Created XML request: {}", xmlRequest)
                transaction.getLinkActionInProgress().setRequest(xmlRequest)
            }
            else
            {
                log.info("\n ==================\n NO MFS TRANSFERS IN REQUEST TO PROCESS for requestId: [{}] - inboundTxnId: [{}]\n ==================", transaction.getInboundRequest().getInboundTxnId(), transaction.getInboundRequest().getInboundTxnId())
                throw new SkippableActionException("NO MFS TRANSFERS IN REQUEST TO PROCESS", LinkActionStatus.SKIPPED)
            }
    ...
    void makeApiCall(HelperTransaction transaction) {
        ...
        try {
            ...
            if(responseEntity.statusCode == HttpStatus.OK || responseEntity.getStatusCode() == HttpStatus.CREATED || responseEntity.getStatusCode() == HttpStatus.ACCEPTED){
    ...
    void callback(HelperTransaction transaction) {
        ...
        try {
            ...
            TransferRequest[] requests = rawRequest.MFSTRANSFER
            ...
            log.info("Order ID: [{}] for requestId: {} with inboundTxnId: {}", request.command.referenceid, requestId, inboundTxId)
            ...
            if(transaction.isExternalCallSuccess())
            {
                ...
                extUpdateOrderRequest.orderId = request.command.referenceid
                extUpdateOrderRequest.resellerId = request.command.buyerid
                ...
                extUpdateOrderRequest.externalActionResult.transferResult.transferType = "MFS"
                extUpdateOrderRequest.externalActionResult.transferResult.isSuccess = true
    ...
    private static Element createCommandElement(Document doc, SystemToken systemToken, TransferRequest transferRequest) {
        ...
        addElement(doc, commandElement, "TYPE","REQAO2C")
        addElement(doc, commandElement, "PROVIDER","101")
        addElement(doc, commandElement, "PROVIDER2","101")
        addElement(doc, commandElement, "PAYID", "12")
        addElement(doc, commandElement, "MSISDN", transferRequest.command.msisdn)
        addElement(doc, commandElement, "AMOUNT", transferRequest.command.amount)
        addElement(doc, commandElement, "REFERENCEID", transferRequest.command.referenceid)
        addElement(doc, commandElement, "SESSION_ID", transferRequest.command.session_id)
        addElement(doc, commandElement, "TRID", transferRequest.command.trid)
    ...
    static class Payload {
        TransferRequest[] MFSTRANSFER
        
        @Override
        String toString() {
            final StringBuilder sb = new StringBuilder("Payload{")
            sb.append("MFSTRANSFER=").append(MFSTRANSFER)
            sb.append('}')
            return sb.toString()
        }
    }
    ...
    static class RequestCommand {
        ...
        String toString() {
            ...
            sb.append(", provider2='").append(provider2).append('\'')
