...
class GPC_ERP_INV_SO_CREATION_TASK extends AbstractAction {
    private static final Logger log = LoggerFactory.getLogger(GPC_ERP_INV_SO_CREATION_TASK.class)
    ...
    private static final String TASK = GPC_ERP_INV_SO_CREATION_TASK.class.name
    @Override
    ...
    void createRequest(HelperTransaction transaction) {
        ...
        try {
            ...
            log.info("Received INV SO REQUEST: {}", request.SALESORDERS.INV)
            if(request.SALESORDERS.INV != null)
            {
                setDefaultSalesOrderVariables(request.SALESORDERS.INV)
                String salesOrderRequest = gson.toJson(request.SALESORDERS.INV)
    ...
    void makeApiCall(HelperTransaction transaction) {
        ...
        try {
            ...
            log.info("INV SO Request to ERPLink: [{}]", salesOrderRequest)
    ...
    void callback(HelperTransaction transaction) {
        ...
        try {
            ...
            CreateSalesOrderRequest INV =  gson.fromJson(request, CreateSalesOrderRequest.class)

            ExtUpdateOrderRequest extUpdateOrderRequest = new ExtUpdateOrderRequest()
            extUpdateOrderRequest.orderId = INV.orderId
            extUpdateOrderRequest.tokenObj = transaction.getToken()
            extUpdateOrderRequest.resellerId = INV.buyingPartyId
            extUpdateOrderRequest.operationId = ExtUpdateOrderOperation.DUMMY_PO_SALES_ORDER
    ...
    static setDefaultSalesOrderVariables(CreateSalesOrderRequest request){
        ...
        request.sourceTransactionSystem = "RMS"
        request.businessUnitName = "Grameenphone Ltd."
        request.paymentTerms = "Immediate"
        request.transactionTypeCode = "GP GPC Sales Order"
...
//            }
    ...
    {
        STATUS, ORDER_QUANTITY, STATUS_AND_QUANTITY, TRANSFER_STATUS, SALES_ORDERS, DUMMY_PO_SALES_ORDER
