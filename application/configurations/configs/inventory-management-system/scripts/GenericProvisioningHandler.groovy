...
class GenericProvisioningHandler implements ProvisioningHandler
{
    ...
    @Value('${inventory.types.hybrid:hybrid}')
    private String hybrid;
    @Value('${inventory.types.hybrid.check:true}')
            ...
            BulkInventoryRequest request, String token)
    {
        ...
        for (BulkInventoryDto inventoryModel : request.getInventories())
        {
            ...
            if (hybridEnabled) {
                if (ProductType.HYBRID.equals(products.get(inventoryModel.getProductSKU()).getType())) {

                    if (inventoryModel.hasSerialNumber()) {
                        products.get(inventoryModel.getProductSKU()).setType(ProductType.SERIALIZED);
                        inventoryModel.setProductType(ProductType.SERIALIZED);
                    } else {
                        products.get(inventoryModel.getProductSKU()).setType(ProductType.NONSERIALIZED);
                        inventoryModel.setProductType(ProductType.NONSERIALIZED);
                    }
                }
            }
            ...
            if (ProductType.NONSERIALIZED.equals(products.get(inventoryModel.getProductSKU()).getType()))
            {
                inventoryModel.setProductType(ProductType.NONSERIALIZED);
            }
            if (ProductType.TRACKABLE_NONSERIALIZED.equals(products.get(inventoryModel.getProductSKU()).getType()))
            {
                inventoryModel.setProductType(ProductType.TRACKABLE_NONSERIALIZED);
            }
        }
                ...
                if (ProductType.SERIALIZED.equals(products.get(inventoryModel.getProductSKU()).getType()))
                {
                    ...
                    for (String serial : serials)
                    {
                        try
                        {
                            ...
                            if (ImportType.valueOf(request.getImportType()) != ImportType.FAIL_ON_ERROR)
                                inventoryItemAdapter.createInventoryWithNewTransactions(inventoryModel);
                            else
                                inventoryItemAdapter.createInventories(inventoryModel);
                        ...
                        catch (Exception e)
                        {
                            ...
                            else if (ImportType.valueOf(request.getImportType()) == ImportType.FULL_IMPORT)
                            {
